...
<div class="cc_wrapper">
    ...
    <table class=" table nowrap data-table table-responsive display " style="width:100%" cellspacing="0"  id="data_table">
     ...
     <td>{{$data->product}}</td>
       ...
       @foreach ($enquiries as $singleenq )
        
      @if($singleenq->id == $data->enquiry_id)
          @foreach ($clients as $client )
         @if($singleenq->organisation_id ==$client->id)     
     ...
     @endforeach
      ...
      <!-- <td>  
         @cannot('isManager')
       <button class="btn btn-success " name="edit" data-product_id="{{$data->id}}"> <i class="fas fa-edit"></i></button>
       <button class="btn btn-danger " name="delete" data-product_id="{{$data->id}}"> <i class="fas fa-trash-alt"></i></button>
         @endcannot
       </td>-->
...
$('#createNewProduct').click(function () {
    ...
    $('button.editproductModal').click(function()
        {
         var followup_id=$(this).attr("data-product_id");
         window.location.href="{{ url('/UpdateForm/display') }}"+'/' + followup_id ;
      });
